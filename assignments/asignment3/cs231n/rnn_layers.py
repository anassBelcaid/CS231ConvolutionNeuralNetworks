...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ## Vanilla formula for  the hidden state
...
def rnn_step_backward(dnext_h, cache):
    ...
    #computing the tanh gradient
    dtanh = 1-np.tanh(hidden)**2
    dtanh= dtanh*dnext_h
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        # h[:,i,:],cache = lsm_step_forward(x[:,i,:], h0, Wx, Wh, b)
        h[:,i,:],c,cache = lstm_step_forward(x[:,i,:],h0,c,Wx,Wh,b)
        h0=h[:,i,:]
        caches[i]=cache
...
def lstm_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        ...
        dx[:,i,:],dh0,dc,dwx,dwh,dbt = lstm_step_backward(dh[:,i,:]+dh0,dc,cache[i])
        dWx += dwx
        dWh += dwh
        db  += dbt
